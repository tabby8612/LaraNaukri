...
export default function ProjectsCard() {
    ...
    async function getProjects() {
        const response = await fetch(route('candidate.projects'));
    ...
    return (
        <Card className="mt-10 border-gray-200 p-7 shadow-xl">
            <div className="flex items-center justify-between">
                ...
                <UploadProject trigger="+" refreshProjectsFn={getProjects} />
            </div>
            <div className="grid grid-cols-4 gap-4">
                {projects.length > 0 ? (
                    projects.map((project) => <PortfilioProject project={project} refreshProjectsFn={getProjects} key={project.id} />)
                ) : (
