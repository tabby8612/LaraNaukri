...
export default function NavItems({ page, navType }: { page: string; navType: 'horizontal' | 'vertical' }) {
    const { auth } = usePage<Props>().props;
    const [userOptions, setUserOptions] = useState(false);

    return (
        <ul className={`flex ${navType === 'vertical' && 'flex-col'} items-center justify-center gap-7`}>
            <a href={route('home')}>
                <li
            ...
            {auth?.user && auth?.user.role === 'candidate' && (
                <li className="relative cursor-pointer" onMouseEnter={() => setUserOptions(true)} onMouseLeave={() => setUserOptions(false)}>
                    <img
            ...
            {!auth?.user && (
                <div className={`flex ${navType === 'vertical' && 'flex-col'} items-center gap-7`}>
                    <li
