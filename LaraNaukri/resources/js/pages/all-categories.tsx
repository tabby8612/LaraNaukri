...
export default function AllCategories() {
    ...
    return (
        <AppLayout page="categories">
            ...
            <section className="mx-auto my-16 grid size-11/12 grid-cols-2 gap-4 md:grid-cols-6">
                {categories &&
                    categories.map((category) => (
                        <JobCategoryCard id={category.id} imageUrl={category.image_path} jobs={category.jobs_count} name={category.name} />
                    ))}
            </section>
        </AppLayout>
    );
